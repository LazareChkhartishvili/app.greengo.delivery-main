...
const useCreateEditForm = (
  ...
  const initialValues: CreateEditProductSchema = {
  ...
  const onSubmit: SubmitHandler<CreateEditProductSchema> = async (data) => {
    try {
      setLoading(true);
      ...
      const response = await api.services.products.createEditProduct(
        token,
        dataToSend,
        productId
      );
      ...
      if (response?.ok) {
        ...
        router.replace(
          id ? routes.company.companyProducts(id) : routes.company.company
        );
        router.refresh();
      }
      if (!response.ok) {
